...
/***/ function(module, exports, __webpack_require__) {
	...
	_dexie2.default.Promise.on('error', function (err) {
	  console.log('dexieErr:', err);
	});

	var db = new _dexie2.default('chatid');
	db.version(1).stores({
	  state: '_id, data'
	});
	      ...
	      switch (change.type) {
	        ...
	        case 2:
	          ...
	          if (wasme) {
	            ...
	            var data = change.obj.data;
	            tell_parent({ action: "broadcast", data: data }, event);
	          }
	          break;
	        case 3:
	          // DELETED
	...
	var debouncedPut = debounce(function (data, event) {
	  wasme = true;
	  db.state.put({ _id: filterOrigin(event.origin), data: data }).catch(function (err) {
	    if (err) {
	...
	function get(event) {
	  ...
	  db.state.where('_id').equals(filterOrigin(event.origin)).toArray(function (docs) {
...
/***/ function(module, exports, __webpack_require__) {
	...
	(function (global, factory) {
	    ...
	    global.Dexie.Observable = factory(global.Dexie);
